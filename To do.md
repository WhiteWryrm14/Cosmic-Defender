Cosmic Defender SDL3: A 1:1 C++26 Port of nFireInvaders.py
...
- docs/Wiki.md: a beginner-friendly walkthrough of every step  
...
Cosmic Defender SDL3: Beginner’s Wiki

This guide breaks down every line of the C++26 + SDL 3 port so you can follow—even if you’ve never coded in C++.

---

1. Prerequisites

- C++ compiler with C++26 (GCC ≥ 13, Clang ≥ 17, MSVC latest)  
- CMake ≥ 3.23  
...
4.1 SDL Initialization

`cpp
SDLInit(SDLINIT_VIDEO);
TTF_Init();
// create window & accelerated renderer
`

- SDL_Init boots up subsystems.  
- TTF_Init readies font rendering.

4.2 Game Loop & Delta-Time
...
- We compute dt (seconds since last frame) so movements are smooth on any framerate.

4.3 Shapes without a Graphics Library

- Filled circle: loop over vertical spans, draw horizontal lines via SDL_RenderDrawLineF.  
- Triangle: use SDL_RenderGeometry to render 3 vertices.  
- Rectangles: SDL_RenderFillRect.

4.4 Entities & Collisions

- Structs store position, size.  
- AABB collision checks if boxes overlap.  
- We mirror Python’s logic: move aliens, bullets, spawn power-ups, detect hits, update score.
...
- Every frame we re-create the text texture.  
- You could optimize by only updating when score/life change.

---

5. Next Steps & Enhancements

1. Refactor to OOP: split Player, Invader, Game into classes  
2. Add Sound: integrate SDL3::SDL3_mixer for blasts & power-ups  
3. Menus & Screens: build title/end screens with text & buttons  
4. Particle Effects: tiny shapes for explosions  
5. Packaging: create install targets in CMake  
